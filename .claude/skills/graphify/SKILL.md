...
**Fast path — graph of another repo:** If the question is about a *different* codebase that has already
been graphed elsewhere (commonly a clone under `~/.graphify/repos/<owner>/<repo>/graphify-out/graph.json`),
do not rebuild anything and do not merge graphs. Pass that file to the read commands with `--graph`:

```bash
graphify query "<question>" --graph <path>/graphify-out/graph.json
```

`path`, `explain`, `affected`, `god-nodes`, and `tree` accept the same flag. `--graph` defaults to
`graphify-out/graph.json` in the current working directory and there is no env var override, so it must
be passed explicitly for any graph outside the cwd. Check which external graphs already exist before
extracting: `ls ~/.graphify/repos/*/*/graphify-out/graph.json`. Project CLAUDE.md may also list them.
