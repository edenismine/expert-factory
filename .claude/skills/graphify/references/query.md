...
### Querying a graph outside the current directory

Every read command defaults to `graphify-out/graph.json` in the cwd and accepts `--graph <path>` to
override it. To query an already-built graph of another repo, point at its file directly — no merge, no
global graph, no re-extraction:

```bash
GRAPH=~/.graphify/repos/<owner>/<repo>/graphify-out/graph.json
...
Substitute that path everywhere `graphify-out/graph.json` appears below, including the existence check
above and the inline NetworkX fallbacks. Vocabulary expansion must read the **same** graph you traverse —
expanding against the local graph while querying an external one reintroduces the wording mismatch this
step exists to prevent.

Two caveats when the graph is not the cwd's:
- `graphify-out/.graphify_python` may not exist next to an external graph, so the inline Python fallbacks
  need an interpreter from elsewhere (this project's `graphify-out/.graphify_python`, or plain `python3`).
- `save-result` writes to `graphify-out/memory/` in the cwd by default. Keep external-graph findings out
  of this project's memory, or aim them at the other repo with `--memory-dir`.
